...
Поиск, настройки, статистика, добавление и открытие всех пока без действий. Кнопка `Open` в карточке Kling уже открывает веб-приложение Kling в системном браузере.

Кнопки в шапке и карточке провайдера используют общий компонент `Button`.

Авторизация Kling остаётся в системном браузере. AI Collector не хранит логин, пароль или cookies. При нескольких аккаунтах используется аккаунт активного профиля браузера.
...
- не подключать Playwright, аккаунты и storage без отдельной задачи;
- не расширять существующий Electron IPC за пределы открытия провайдера без отдельной задачи;
